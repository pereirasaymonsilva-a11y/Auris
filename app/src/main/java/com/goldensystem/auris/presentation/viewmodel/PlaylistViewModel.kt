...
import com.goldensystem.auris.data.model.SmartPlaylistRule
import com.goldensystem.auris.data.model.Song
import com.goldensystem.auris.data.model.SortOption
import com.goldensystem.auris.data.playlist.M3uManager
...
import com.goldensystem.auris.data.preferences.TelegramTopicDisplayMode
...
class PlaylistViewModel @Inject constructor(
    ...
    private fun findFolder(
        targetPath: String,
        folders: List<com.goldensystem.auris.data.model.MusicFolder>
    ): com.goldensystem.auris.data.model.MusicFolder? {
        val queue: ArrayDeque<com.goldensystem.auris.data.model.MusicFolder> = ArrayDeque(folders)
    ...
    private fun com.goldensystem.auris.data.model.MusicFolder.collectAllSongs(): List<Song> {
    ...
    private fun sortPlaylistsList(
        playlists: List<com.goldensystem.auris.data.model.Playlist>,
        sortOption: SortOption
    ): List<com.goldensystem.auris.data.model.Playlist> {
        return when (sortOption) {
            SortOption.PlaylistNameAZ -> playlists.sortedWith(
                compareBy<com.goldensystem.auris.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistNameZA -> playlists.sortedWith(
                compareByDescending<com.goldensystem.auris.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreated -> playlists.sortedWith(
                compareByDescending<com.goldensystem.auris.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreatedAsc -> playlists.sortedWith(
                compareBy<com.goldensystem.auris.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
    ...
    fun exportPlaylistsAsM3u(playlistIds: List<String>) {
        ...
        viewModelScope.launch {
            try {
                ...
                val exportDir = File(musicDir, "Auris Exports")
                if (!exportDir.exists()) {
                    exportDir.mkdirs()
                }
